...
This conundrum can also be visualized in a phylogenetic context by rooting a tree of early SARS-CoV-2 sequences so that the ``progenitor'' sequence is chosen to be closest to the bat coronavirus outgroup.
If we limit the analysis to sequences with at least two observations among strains collected no later than January 2020, there are three possible ways to root the tree in this fashion (Figure~\ref{fig:tree_RaTG13}, Figure~\ref{suppfig:tree_RpYN06_RmYN02}).
