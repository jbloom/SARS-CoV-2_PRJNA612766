...
The discrepancy is especially pronounced for early Wuhan sequences collected from patients who had visited the Huanan Seafood Market, which was widely reported as an initial location of the outbreak and has been postulated to be a site of potential zoonosis~\citep{WHO2021origins}.
...
Importantly, all these observations also hold true if SARS-CoV-2 is compared to other related bat coronaviruses~\citep{lytras2021exploring} such as RpYN06~\citep{zhou2021identification} or RmYN02~\citep{zhou2020novel} rather than RaTG13 (Supplementary Figure~S\ref{suppfig:deltadist_RpYN06_RmYN02}).

This conundrum can also be visualized in a phylogenetic context by rooting a tree of early SARS-CoV-2 sequences so that the ``progenitor'' sequence is chosen to be the one closest to the bat coronavirus outgroup.
If we limit the analysis to sequences with at least two observation among strains collected no later than January 2020, there are three possible ways to root the tree 
 (Figure~\ref{fig:tree_RaTG13} shows these rootings using a RaTG13 outgroup; Supplementary Figure~S\ref{suppfig:tree_RpYN06_RmYN02} shows identical results are obtained using RpYN06 or RmYN02 outgroups).
